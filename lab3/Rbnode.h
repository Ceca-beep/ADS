...
struct RBNode {
    ...
    std::string toString() {
        return std::to_string(key) + (color == RED ? "(R)" : "(B)");
    }
};
...
public:
    ...
    void findMax(RBNode* n, Color c, int& m) {
        if (n == nil) return;
        if (n->color == c) m = std::max(m, n->key);
        findMax(n->left, c, m); findMax(n->right, c, m);
    }

    void indentedDisplay(RBNode* n, unsigned int s) {
        if (n == nil) return;
        indentedDisplay(n->right, s + 7);
        std::string spaces;
        for(unsigned int i = 0; i < s; ++i) { spaces += ' '; }
        std::cout << spaces << n->key << (n->color == RED ? "R" : "B") << std::endl;
    ...
    // ── Used by DynamicMedian ────────────────────────────────────
    int countNodes(RBNode* n) {
        if (n == nil) return 0;
        return 1 + countNodes(n->left) + countNodes(n->right);
    }

    RBNode* kthSmallest(int k) {
...
private:
    ...
    RBNode* kthSmallestHelper(RBNode* n, int& k) {
        if (n == nil) return nil;
        RBNode* left = kthSmallestHelper(n->left, k);
